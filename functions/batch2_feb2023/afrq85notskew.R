...
#####################
### n1000 !skewd - allelic frq = 85: ###
#####################


#Set Output file name:
outputfile = "./simulations_afrq85_notskew_table.csv"
summaryfile = "./summary_afreq85_notskew.csv"
...
for( m in mproportion){
  for (sd in sdproportion) {
    for (j in 1:nsim) {
      data = run_model(meffect=m, sdeffect=sd, skew=sk, p = 0.85)
      if (is.list(data)){
