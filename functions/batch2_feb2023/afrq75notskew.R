...
#####################
### n1000 !skewd - allelic frq = 75: ###
#####################


#Set Output file name:
outputfile = "./simulations_afrq75_ notskew_table.csv"
summaryfile = "./summary_afreq75_notskew.csv"
...
for( m in mproportion){
  for (sd in sdproportion) {
    for (j in 1:nsim) {
      data = run_model(meffect=m, sdeffect=sd, skew=sk, p = 0.75)
      if (is.list(data)){
