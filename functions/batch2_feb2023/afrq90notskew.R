...
#####################
### n1000 !skewd - allelic frq = 90: ###
#####################


#Set Output file name:
outputfile = "./simulations_afrq90_notskew_table.csv"
summaryfile = "./summary_afreq90_notskew.csv"
...
for( m in mproportion){
  for (sd in sdproportion) {
    for (j in 1:nsim) {
      data = run_model(meffect=m, sdeffect=sd, skew=sk, p = 0.90)
      if (is.list(data)){
