#Example to run regression models
#Jan 2023
#By Xochitl Diaz
source("./genemodel.R")
...
### Run example: ###
####################
#Don't actually run this it will take a lot of time

#Set Output file name:
outputfile = "./test_table.csv"
summaryfile = "./summary_table.csv"

sdproportion = seq(from = 0.2, to= 3, by = 0.2)
...
for( m in mproportion){
  for (sd in sdproportion) {
    for (j in 1:nsim) {
      data = run_model(meffect=m, sdeffect=sd, skew=sk)
      if (is.list(data)){
    ...
    summary_models[nrow(summary_models)+1,]= c(m , sd, sk, 
                                               ...
                                               length(which(this_model$quali_pval  < 0.05))/nsim)
